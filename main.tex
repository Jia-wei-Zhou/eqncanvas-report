...
example, stroke \code{\{\{1, 1\}, \{2, 2\}, \{3, 3\}\}} means a line starting at point (1, 1), passing point (2, 2) and ends at (3, 3). All paths drawn on the canvas can be expressed as stroke data. An example of a stroke and its stroke data is shown in Figure \ref{fig:stroke-sample}.
\begin{figure}
    \centering
...
\code{handleMouseDown}, \code{handl-} \code{eMouseMove} and \code{handleMouseUp}
...
position. This is done by methods \code{moveto(previou-} \code{sPoint)} and
...
\code{handle-} \code{PointerMove} and \code{handlePointerUp}, which have same
...
to \code{stroke-} \code{Upload} with the data structure required by \textit{Mathpix}. This function is invoked in \code{undo}, \code{redo}, \code{remove} and \code{handleMouseUp}.
...
To do this, the programme was slightly refactored. Before, a stroke has a type of \code{Point[]}. However, to store the border position of a stroke, data of maximum and minimum coordinates need to be added. Therefore, a new type \code{Stroke} is created, which has \code{Point[]} representing points of the stroke, and \code{minX}, \code{minY}, \code{maxX}, \code{maxY} to shape the borders of the stroke. 

Another change is that, the current stroke data is added to \code{strokeList} in
\code{handlePo-} \code{interDown}. However, updates of stroke data is done in
\code{handlePointerMove} to renew border data, and \code{strokeList} is affected
as well. Frequent updates will affect the performance of the component.
Therefore, adding the stroke data to \code{strokeList} is removed to
\code{handlePointerUp}, so that \code{handlePointerMove} does not need to renew
\code{strokeList} every time. This thus requires the current stroke to be
rendered independently. It is done by creating a new \code{Path} in sub-component \code{Canvas}. This as well reduces the load of the page as it only needs to rerender the current stroke instead of the whole \code{strokeList}.
